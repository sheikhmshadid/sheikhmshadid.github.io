...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.loage-project').evaluate(el => getComputedStyle(el).backgroundColor), 'rgb(26, 4, 3)');
    assert.equal(await page.locator('.studio-project').evaluate(el => getComputedStyle(el).backgroundColor), 'rgb(252, 252, 252)');
    const popupReady = page.waitForEvent('popup');
    await page.getByRole('link', { name: 'Open full-size Loage screenshot', exact: true }).click();
    const popup = await popupReady;
    await popup.waitForLoadState();
    assert(popup.url().endsWith('/assets/loage-screenshot.png'));
    await popup.close();
